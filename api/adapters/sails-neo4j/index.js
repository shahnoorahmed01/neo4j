...
const neo4j = require('neo4j-driver')
    ...
    // const driver = await neo4j.driver('bolt://localhost:7687', neo4j.auth.basic('neo4j', 'm6rznbzwk7k'))
    // let manager = await driver.session()
    // manager = await driver.session({database:"superone"});
...
module.exports = {
   ...
   */
   ...
   // nativequery: function (datastoreName, query, done) {
  ...
  find: function (datastoreName, query, done) {
    ...
    let driver =dsEntry.driver
    let queryData;
    const newQuery =`MATCH (n:Member) RETURN n LIMIT 20`
    
    session.run(newQuery).then((result) => {
        return done(result);
      })
